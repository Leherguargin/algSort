...
export default class Wykresy extends React.Component {
    ...
    handleInputChange(event) {
        ...
        alg[event.target.name] = event.target.value;
        this.setState({
    ...
    componentDidMount() {
        fetch("http://inzmlback.herokuapp.com/sort", {
            //http://localhost:8091/sort
            method: "POST",
            headers: {
    ...
    render() {
        ...
        if (error) {
            ...
            let wykresik = <section className="col-md-10"><div style={{color: "white"}} >Ładowanie...</div></section>;
            if(isLoaded && (this.state.wyswietlaneAlgorytmy[0] || this.state.wyswietlaneAlgorytmy[1])){
                wykresik = (<section className="col-md-10">
                    <Wykres
